...
let getLongestValid = () => {
	let ret = [];
	let curhash = longest_hash;

	while (curhash != genesis) {
		ret.push(blockchain[curhash]);
		curhash = blockchain[curhash]["content"]["prevhash"];
	}
...
app.get("/", (req, res) => {
	res.send(`I am a blockchain node!
		<h1>Peers</h1>
		${JSON.stringify(peers)}<h1>Received Blocks</h1>
		${JSON.stringify(blockchain)}<h1>Longest valid</h1>
		${JSON.stringify(getLongestValid())}`
	);
});
